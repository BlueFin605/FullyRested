...
export class ExecuteRestCallsService {
  ...
  replaceVariables(text: string, solution: Solution | undefined, overrideVariables: VariableTable[] | undefined, overrideSecrets: SecretTable[] | undefined): string {
    ...
    console.log(matches);
    matches.forEach(m => {
      text = this.substituteValue(text, m[0],m[1], solution, overrideVariables, overrideSecrets);
    });
  ...
  findVariable(value: string, solution: Solution | undefined, overrideVariables: VariableTable[] | undefined, overrideSecrets: SecretTable[] | undefined): string {
    ...
    if (value.startsWith('$')) {
      ...
      overrideVar = overrideSecrets?.find(v => v.$secret == value)?.$value;
      ...
      overrideVar = overrideVariables?.find(v => v.variable == value)?.value;
